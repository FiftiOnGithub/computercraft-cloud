    ...
    for i = 2, #image do
        ...
        for i = 1, temp.height do
            ...
            if a1 ~= pa1 or b1 ~= pb1 or c1 ~= pc1 then
                ...
                newImage[1][i], newImage[2][i], newImage[3][i], newImage.height =
                    min > 1 and { min - 1, a1:sub(min, max) } or a1:sub(min, max), b1:sub(min, max), c1:sub(min, max), i
            end
...
function createWindow(parent, x, y, width, height, visible)
    ...
    window.blit = function(_, _, bC)
        ...
        if visible then
            ...
            frontBuffer[rowNum] = { thisRow[1]:sub(1, firstColNum - 1) .. chars1 .. thisRow[1]:sub(lastColNum + 1),
                thisRow[2]:sub(1, firstColNum - 1) .. chars2 .. thisRow[2]:sub(lastColNum + 1), thisRow[3]:sub(1,
                firstColNum - 1) .. chars3 .. thisRow[3]:sub(lastColNum + 1) }
        else
    ...
    window.clear = function()
        ...
        for y = 1, cHeight do
            workBuffer[y], backBuffer[y], frontBuffer[y] = { ["max"] = 0 }, newLine(cWidth, bCol),
                { t, fC, bC }
        end
        window.redraw()
    end
    ...
    window.scroll = function(lines)
        ...
        if lines ~= 0 then
            if lines % 3 == 0 then
                ...
                for y = 1, cHeight do
                    newWB[y], newBB[y], newFB[y] = workBuffer[y + lines] or { ["max"] = 0 },
            ...
            for i = 1, cHeight do
                ...
                if work.max > 0 then
                    ...
                    if count < cWidth + 1 then
                        line1[lineLen], line2[lineLen], line3[lineLen] = front1:sub(count),
                            front2:sub(count), front3:sub(count)
                    end
        ...
        if type(newHeight) == "number" then
            ...
            if newHeight > cHeight then
                ...
                for y = cHeight + 1, newHeight do
                    workBuffer[y], backBuffer[y], frontBuffer[y] = { ["max"] = 0 },
            ...
            for i = 1, height do
                if thisImage[1][i] then
                    ...
                    elseif rowType == "table" then
                        ...
                        error("Malformed row record #" ..
                            i .. " in frame #" .. k .. " when attempting to save \"" ..
